...
describe("UsePhotoFetcher", () => {
  ...
  it("calls getPhotos api with initial query", async () => {
    ...
    expect(mockgetPhotos).toHaveBeenCalledWith(expect.objectContaining({ query, page: 1 }), expect.anything());
  });
  ...
  it("fetches new photos when pagination change", async () => {
    ...
    expect(mockgetPhotos).toHaveBeenCalledWith(expect.objectContaining({ query, page: newPage }), expect.anything());
  });
...
function renderPhotoFetcher(initialQuery: string) {
  return renderHook(({ query }) => usePhotoFetcher(query, 10), { initialProps: { query: initialQuery } });
}
